<template>
  <TemplateModal>
    <template v-slot:default>
      <div class="m-modal-title">{{ $t("MODAL.SHOW_EDIT_WF.EDIT_WF") }}</div>
      <div class="form-group row mx-0">
        <b-form-group class="radio-group" v-slot="{ ariaDescribedby }">
          <b-form-radio v-model="selected" class="my-3" :aria-describedby="ariaDescribedby" value="1"
            >{{ $t("MODAL.SHOW_EDIT_WF.CURRENT_EDITING") }}
          </b-form-radio>
          <b-form-radio
            v-model="selected"
            ...
            value="2"
            >{{ $t("MODAL.SHOW_EDIT_WF.CONTENT_AND_VERSION") }}
          </b-form-radio>
        </b-form-group>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.versionList = [{ value: 0, text: this.$t("MODAL.SHOW_EDIT_WF.CHOOSE_VERSION") }];
    ...
    methods: {
      ok() {
        if (this.selected == 2 && !this.version) return this.$alert(this.$t("MODAL.SHOW_EDIT_WF.CHOOSE_VERSION_ALERT"));
        let version = (this.selected == 2 && this.version) || null;
        this.$emit("ok", this.selectedData.uuid, version);
      },
