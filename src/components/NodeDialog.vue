<template>
  <div>
    <div class="workflow-modal" v-if="visible">
      <div class="header">
        <span>Edit</span>
      </div>
      <div class="body">
        ...
        <select class="form-control" id="type" v-model="nodeForm.type">
          <option :key="'node-type-' + item.id" :value="item.id" v-for="item in nodes">
            {{ item.name }}
          </option>
        </select>
        <label for="approver">Approver</label>
        <select class="form-control" id="approver" :value="nodeForm.approver.id" @change="handleChangeApprover($event)">
...
<script>
  import "~@/assets/scss/WorkflowModal.scss";
