  ...
  <div class="location-management">
    ...
    <div class="main-content" :class="{ maximized: isSidebarMinified }">
      <div class="upper-section mx-auto">
        <button class="btn create-btn mt-4 float-right" @click="showModal('createLocation')">
                ...
                <tr class="m-tr tr-list" :key="index">
                  ...
                  <td class="m-td">
                    <div
                      ...
                      role="button"
                      @click="showModal('updateLocation', item)"
    ...
    <CreateLocation
      v-if="modalType.createLocation"
      @ok="closeModal('createLocation', true)"
      @cancel="closeModal('createLocation', false)"
    ></CreateLocation>

    <UpdateLocation
      v-if="modalType.updateLocation"
      :selectedData="selectedData"
      @ok="closeModal('updateLocation', true)"
      @cancel="closeModal('updateLocation', false)"
    ></UpdateLocation>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      getLocationList() {
        ...
        this.apiGet("equipment/position_list", data).then(({ data }) => {
          this.locationList = data.positions;
          this.totalCount = data.total;
        });
      },

      deleteLocation(position_uuid) {
        this.apiPost("equipment/delete_position", { position_uuid }).then(({ data }) => {
          this.getLocationList();
        });
      },
      ...
      closeModal(type, reload) {
        if (reload) this.getLocationList();
      ...
      confirmDeleteLocation(item) {
        this.$confirm(this.$t("LOCATION.DELETE_LOCATION"), this.$t("LOCATION.DELETE_LOCATION_CONFIRM"), () => {
          this.deleteLocation(item.position_uuid);
        });
      },
    },
  };
