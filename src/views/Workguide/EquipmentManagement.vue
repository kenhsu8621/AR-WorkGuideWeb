  ...
  <div class="equipment-management">
    ...
    <div class="main-content" :class="{ maximized: isSidebarMinified }">
      <div class="upper-section mx-auto">
        <button class="btn create-btn mt-4 float-right" @click="showModal('upsertEquipment')">
          {{ $t("EQ.ADD_EQUIPMENT") }}
        </button>
      </div>
          ...
          <table class="m-table">
            <thead>
              <tr class="m-tr tr-item">
                ...
                <th class="m-th">
                  <div class="m-th-content">
                    <span>{{ $t("EQ.EQUIPMENT_LOCATION") }}</span>
                  </div>
                </th>
                <th class="m-th">
                  <div class="m-th-content">
            ...
            <tbody>
              <template v-for="(item, index) in equipmentList">
                <tr class="m-tr tr-list" :key="index">
                  ...
                  <td class="m-td">{{ item.position_name }}</td>
                  <td class="m-td">
                    <div
                    ...
                    <div
                      ...
                      role="button"
                      @click="showModal('upsertEquipment', item)"
                      v-b-tooltip.hover.bottom
                    ...
                    <div
                      ...
                      role="button"
                      @click="confirmDeleteEquipment(item)"
                      v-b-tooltip.hover.bottom
    ...
    <canvas class="qrcode-cv d-none" id="qrcodeCv" ref="qrcodeCv" width="400" height="440"></canvas>

    <UpsertEquipment
      v-if="modalType.upsertEquipment"
      :typeList="typeList"
      :locationList="locationList"
      :selectedData="selectedData"
      @ok="closeModal('upsertEquipment', true)"
      @cancel="closeModal('upsertEquipment', false)"
    ></UpsertEquipment>
  </div>
</template>
...
<script>
  ...
  import UpsertEquipment from "@/components/modal/UpsertEquipment";
  ...
  export default {
    ...
    methods: {
      getEquipmentList() {
        ...
        this.apiGet("equipment/list", data).then(({ data }) => {
      ...
      getTypeList() {
        this.apiGet("equipment/type_list").then(({ data }) => {
          this.typeList = [{ value: 0, text: this.$t("EQ.CHOOSE_EQUIPMENT_TYPE") }];
          for (let type of data.types) {
            this.typeList.push({ value: type.type_uuid, text: type.type_name });
          }
        });
      },

      getLocationList() {
        this.apiGet("equipment/position_list").then(({ data }) => {
          this.locationList = [{ value: 0, text: this.$t("EQ.CHOOSE_LOCATION") }];
          for (let position of data.positions) {
            this.locationList.push({ value: position.position_uuid, text: position.position_name });
          }
        });
      },

      deleteEquipment(equipment_uuid) {
        this.apiPost("equipment/delete", { equipment_uuid }).then(({ data }) => {
          this.getEquipmentList();
        });
      },
      ...
      closeModal(type, reload) {
        if (reload) this.getEquipmentList();
      ...
      downloadQRCode(item) {
        let qrcode = this.$refs[item.equipment_uuid];
        if (!qrcode) return;
        ...
        img.onload = async () => {
          ...
          ctx.font = "20px Arial";
          ctx.fillStyle = "#666";
          ctx.fillText(item.serial, 200, 425);

          let a = document.createElement("a");
          a.href = await canvas.toDataURL("image/jpeg");
          a.download = item.type_name + "_" + item.serial + ".jpg";
      ...
      confirmDeleteEquipment(item) {
        this.$confirm(this.$t("EQ.DELETE_EQUIPMENT"), this.$t("EQ.DELETE_EQUIPMENT_CONFIRM"), () => {
          this.deleteEquipment(item.equipment_uuid);
        });
      },
    },
  };
