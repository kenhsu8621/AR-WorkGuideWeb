<template>
  <div class="feedback-contact-us">
    <Searchbar @search="searchContactUsList" />
    <div class="filter-section">
      <b-form-select
        v-model="selectedEnterprise"
        :options="enterpriseList"
        class="select-component ml-3 my-4"
        @change="getContactUsList(selectedEnterprise)"
      ></b-form-select>
    </div>
      ...
      <table class="m-table mx-auto">
        <thead>
          <tr class="m-tr tr-item">
            ...
            <th class="m-th">
              <div class="m-th-content"><span>Email</span></div>
            </th>
            <th class="m-th">
              <div class="m-th-content"><span>其他意見</span></div>
            </th>
            <th class="m-th">
              <div class="m-th-content"><span>聯繫時間</span></div>
            </th>
            <th class="m-th">
        ...
        <tbody>
          <tr class="m-tr tr-list" v-for="(item, index) in contactUsList" :key="index">
            <td class="m-td">
              <span class="m-value">
                {{ item.enterprise }}
              </span>
            ...
            <td class="m-td">
              ...
              <img
                ...
                class="mx-1 photo-img float-left"
                src="~@/assets/images/management/image.svg"
              />
...
<script>
  ...
  export default {
    ...
    methods: {
      GetDatetimeString,
      getContactUsList(code) {
        let data = {
          code: code == 0 ? null : code,
        };
        this.apiGet("si/contact_us_list", data).then(({ data }) => {
          this.contactUsList = data.list;
        });
      },
      searchContactUsList(keyword) {
        let data = {
      ...
      closeModal(type, reload) {
        if (reload) this.getContactUsList();
