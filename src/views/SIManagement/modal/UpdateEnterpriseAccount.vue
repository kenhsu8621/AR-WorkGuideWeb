<template>
  <TemplateModal :width="33">
    <template v-slot:default>
      <div class="m-modal-title text-center">編輯企業用戶</div>
      <form @submit.prevent="updateEnterpriseAdmin">
        <div class="ra-form-group row">
          <div class="col-2"></div>
          <div class="col-8 row">
            <div class="col-3 m-label text-right">所屬企業</div>
            <div class="col-9 m-text text-left p-0">{{ selectedData.enterprise }}</div>
          </div>
          <div class="col-2"></div>
        </div>
        <div class="ra-form-group row">
  ...
  export default {
    ...
    methods: {
      updateEnterpriseAdmin() {
        ...
        this.apiPost("si/update_enterprise_admin", data).then(({ data }) => {
          this.$emit("ok");
        });
      },
      clearNewPw() {
